...
import { useAuthStore } from "../../../shared/ui/stores/useAuthStore";
...
export function useAuth(): UseAuthReturn {
  ...
  const sendPasswordReset = useCallback(_sendPasswordReset, [_sendPasswordReset]);
  const clearError        = useCallback(_clearError,        [_clearError]);

  return { user, loading, error, signIn, signOut, register, signInWithGoogle, sendPasswordReset, clearError };
}
